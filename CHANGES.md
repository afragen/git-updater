#### [unreleased]
* sanitize array key with `sanitize_title_with_dashes()` not `sanitize_file_name()`, for underscores in key. Sometime other devs hook into `sanitize_file_name` filter and don't unhook causing problems.
