#### [unreleased]
* optimize `ensure_download_data()` — a fast path returns immediately when the download link is already resolved and no dev-asset filter is active (the common admin-page case), skipping all cache reads; otherwise the method reads only the cache columns it needs in a single projected query instead of issuing multiple per-column reads
* fix non-dev release asset download link preferring branch over tag — `ensure_download_data()` no longer recomputes and replaces the branch-or-tag zipball URL that `construct_download_link()` already set on `$repo->download_link`; only a resolved release asset (stable or dev) overrides it, so the tag URL is preserved when a tag is present
* fix `gu_dev_release_asset` one-sided asset maps — `ensure_download_data()` no longer requires both stable and dev asset maps to be populated; a dev-only repo (empty `assets`) now resolves the dev asset instead of falling through to the zipball, and a stable-only repo still resolves the stable asset
* fix `gu_dev_release_asset` download overriding an existing link — `ensure_download_data()` now evaluates the dev release asset even when the repo object already carries a zipball or stable `download_link`, so the dev asset URL wins when newer instead of the consumer listing the stale zipball; a non-dev or older-dev link is left untouched
* fix `gu_dev_release_asset` handling in cache-only consumers — `ensure_download_data()` now honors the dev-release-asset filter (picking the dev asset URL when newer, matching `construct_download_link()`) and sets `newest_tag` to the dev version when the filter is active, so `update_site_transient()` / `plugins_api()` / `themes_api()` serve the dev asset on cache-only requests; also guard against a non-scalar `release_asset_download` and a missing `db_version` option in `GU_Upgrade::run()`
