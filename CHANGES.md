...
* cache: isolate the WordPress object-cache tier into a dedicated `Repo_Cache_Object` class and fix cross-request staleness on persistent object caches (Redis/Memcached) — projected repo data and the row timeout are now keyed by a per-slug generation counter that is bumped on every write, so a write invalidates a slug's whole cache set without enumerating keys (the old per-request key-tracking array silently failed to delete keys across requests, serving stale data until the TTL lapsed). `Abstract_Cache_Table` now delegates all `wp_cache_*` interaction to the new class and keeps only DB concerns; bulk methods (uninstall/delete-all/prune) enumerate slugs and bump each generation. The object cache degrades gracefully on non-persistent hosts — it never throws, every miss falls through to the DB, and the DB remains authoritative
* cache: back projected/cached repo reads (and the row timeout) with the WordPress object cache so repeated reads within and across requests skip redundant DB queries on persistent-cache hosts. Projection keys are order-independent (permutations of a column set share one entry) and scalar vs. array projections stay distinct; the cache is invalidation-safe via the per-slug generation counter above. New `Repo_Cache_Object` class plus `tests/test-repo-cache-object.php`
* perf: `Rest_Update::update_plugin()` now looks the plugin up by array key instead of scanning the config, and `REST_API` reads the `git_updater_additions` option once per request (passing it into `get_api_data()`/`has_uses_lite()`) instead of re-reading it per slug
* security: default-deny private repositories on every REST API route — `plugins-api`, `themes-api`, `update-api` and `download-token` now withhold a genuinely private repo unless the caller presents the shared REST API key or an authorized Lite client domain for that slug. Previously only the Additions "Private Package" hard block was enforced, so a private repo that was never added on the Additions tab was treated as public and returned a signed download URL that `proxy_download()` replayed with the site's stored auth token. `private_package` remains a hard block for every caller, and public packages are unaffected. **Requires git-updater-lite 3.1.0 or newer**, which sends `X-GU-Site-Domain` on the `update-api` request; earlier clients send no headers on that route and are denied for private packages. Staged rollouts are supported via the `gu_enforce_private_package_gate` filter (default `true`), applied consistently to `get_api_data()` and `get_download_token()` so the two cannot disagree
* security: close a first-request authentication bypass on `repos`, `flush-repo-cache` and `reset-branch`. `ensure_api_key_is_set()` now assigns the generated key back to the class property, and the three handlers read `get_site_option( 'git_updater_api_key' )` and reject an empty key *before* `hash_equals()`. Previously the stale empty static made `hash_equals( '', '' )` true on the first request after install or after the option was deleted, allowing unauthenticated repo enumeration, cache flushes and branch resets until the key was loaded
* security: the Remote Management REST API key reset now requires a `POST` request in addition to the existing nonce and `manage_options`/`manage_network_options` checks, so a crafted GET can no longer rotate the shared key (`reset_api_key()` reads `$_POST`, and the reset form posts its `tab`/flag fields in the body)
* security: zipfile remote installs now require an `https` URL on an allowed host instead of accepting any posted URI as the upgrader's `download_link`. The allow-list is filterable via `gu_install_allowed_hosts` and defaults to `github.com`, `gitlab.com`, `bitbucket.org`, the configured Gitea server, any configured self-hosted/enterprise host, and the site's own host. A rejected URL aborts the install instead of running the upgrader
* security: stop writing the expected HMAC signature and secret length to the error log on a download-signature mismatch; that detail is now behind the `gu_debug_download_signature` filter (slug and expiry are still logged)
* fix: `API::add_meta_repo_object()` coerces a missing `private` key in the repo meta response to `false`, removing a PHP warning on hosts whose meta parser omits it
