...
* optimize `ensure_download_data()` — a fast path returns immediately when the repo object is already hydrated (download_link + newest_tag set, no dev-asset filter), skipping all cache reads; otherwise a single full-row read serves the whole method instead of multiple per-column queries
