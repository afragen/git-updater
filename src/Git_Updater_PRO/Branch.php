...
class Branch {
	...
	public function single_install_switcher( $theme ) {
		...
		if ( '1' === self::$options['branch_switch'] ) {
			printf(
				...
				'<p>' . esc_html__( 'Current branch is `%1$s`, try %2$sanother version%3$s', 'gi-updater' ),
			...
			<div id="gu_versions" style="display:none; width: 100%;">
				...
				<option value=""><?php esc_html_e( 'Choose a Version', 'gi-updater' ); ?>&#8230;</option>
			<?php
			...
			if ( empty( $theme->rollback ) ) {
				echo '<option>' . esc_html__( 'No previous tags to rollback to.', 'gi-updater' ) . '</option></select></label>';
			}
			?>
					</select></label>
				<a style="display: none;" class="button-primary" href="?"><?php esc_html_e( 'Install', 'gi-updater' ); ?></a>
			</div>
			<?php
		}
	...
	public function make_branch_switch_row( $data, $config ) {
		...
		printf(
			/* translators: 1: branch name, 2: jQuery dropdown, 3: closing tag */
			esc_html__( 'Current branch is `%1$s`, try %2$sanother version%3$s', 'gi-updater' ),
		...
		if ( null !== $data['branches'] ) {
			foreach ( array_keys( $data['branches'] ) as $branch ) {
				printf(
					'<li><a href="%s%s" aria-label="' . esc_html__( 'Switch to branch ', 'gi-updater' ) . esc_attr( $branch ) . '">%s</a></li>',
		...
		if ( ! empty( $rollback ) ) {
			...
			foreach ( $rollback as $tag ) {
				printf(
					'<li><a href="%s%s" aria-label="' . esc_html__( 'Switch to release ', 'gi-updater' ) . esc_attr( $tag ) . '">%s</a></li>',
					esc_url( $data['nonced_update_url'] ),
		...
		if ( empty( $rollback ) ) {
			echo '<li>' . esc_html__( 'No previous tags to rollback to.', 'gi-updater' ) . '</li>';
		}
