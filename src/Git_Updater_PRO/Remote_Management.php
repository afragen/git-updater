...
class Remote_Management {
	...
	public function add_settings_tabs() {
		$install_tabs = [ 'git_updater_remote_management' => esc_html__( 'Remote Management', 'gi-updater' ) ];
		add_filter(
	...
	public function add_admin_page( $tab, $action ) {
		if ( 'git_updater_remote_management' === $tab ) {
			...
			<form class="settings no-sub-tabs" method="post" action="<?php esc_attr_e( $reset_api_action ); ?>">
				<?php submit_button( esc_html__( 'Reset REST API key', 'gi-updater' ) ); ?>
			</form>
			<?php
		}
	}
	...
	public function remote_management_page_init() {
		...
		add_settings_section(
			'remote_management',
			esc_html__( 'Remote Management', 'gi-updater' ),
	...
	public function print_section_remote_management() {
		...
		echo '<p>';
		esc_html_e( 'Remote Management services should just work for plugins like MainWP, ManageWP, InfiniteWP, iThemes Sync and others.', 'gi-updater' );
		echo '</p>';
		...
		printf(
			wp_kses_post(
				...
				__( 'The <a href="%s">Git Remote Updater</a> plugin was specifically created to make the remote management of Git Updater supported plugins and themes much simpler. You will need the Site URL and REST API key to use with Git Remote Updater settings.', 'gi-updater' )
			),
		...
		printf(
			wp_kses_post(
				...
				__( 'Site URL: %1$s<br> REST API key: %2$s', 'gi-updater' )
			),
		...
		printf(
			wp_kses_post(
				...
				__( 'Please refer to the <a href="%1$s">wiki</a> for complete list of attributes. REST API endpoints for webhook updating begin at: %2$s', 'gi-updater' )
			),
