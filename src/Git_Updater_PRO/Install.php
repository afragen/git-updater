...
class Install {
	...
	public function add_settings_tabs() {
		...
		if ( current_user_can( 'install_plugins' ) ) {
			$install_tabs['git_updater_install_plugin'] = esc_html__( 'Install Plugin', 'gi-updater' );
		}
		if ( current_user_can( 'install_themes' ) ) {
			$install_tabs['git_updater_install_theme'] = esc_html__( 'Install Theme', 'gi-updater' );
		}
		add_filter(
	...
	public function install( $type, $config = null ) {
		...
		if ( isset( $_POST['option_page'] ) && 'git_updater_install' === $_POST['option_page'] ) {
			...
			if ( empty( $_POST['git_updater_repo'] ) ) {
				echo '<h3>';
				esc_html_e( 'A repository URI is required.', 'gi-updater' );
				echo '</h3>';
	...
	public function create_form( $type ) {
		...
		<form method="post">
			...
			if ( 'plugin' === $type ) {
				submit_button( esc_html__( 'Install Plugin', 'gi-updater' ) );
			}
			if ( 'theme' === $type ) {
				submit_button( esc_html__( 'Install Theme', 'gi-updater' ) );
			}
			?>
		</form>
		<?php
	...
	public function register_settings( $type ) {
		...
		if ( 'plugin' === $type ) {
			$repo_type = esc_html__( 'Plugin', 'gi-updater' );
		}
		if ( 'theme' === $type ) {
			$repo_type = esc_html__( 'Theme', 'gi-updater' );
		}
		...
		add_settings_section(
			...
			sprintf( esc_html__( 'Git Updater Install %s', 'gi-updater' ), $repo_type ),
			[],
			'git_updater_install_' . $type
		);
		...
		add_settings_field(
			$type . '_repo',
			/* translators: variable is 'Plugin' or 'Theme' */
			sprintf( esc_html__( '%s URI', 'gi-updater' ), $repo_type ),
			[ $this, 'get_repo' ],
			'git_updater_install_' . $type,
			$type
		);
		...
		add_settings_field(
			$type . '_branch',
			esc_html__( 'Repository Branch', 'gi-updater' ),
			[ $this, 'branch' ],
			'git_updater_install_' . $type,
			$type
		);
		...
		add_settings_field(
			$type . '_api',
			esc_html__( 'Remote Repository Host', 'gi-updater' ),
			[ $this, 'install_api' ],
			'git_updater_install_' . $type,
			$type
		);
	...
	public function get_repo() {
		...
		<label for="git_updater_repo">
			...
			<span class="description">
				<?php esc_html_e( 'URI is case sensitive.', 'gi-updater' ); ?>
			</span>
		</label>
		<?php
	}
	...
	public function branch() {
		...
		<label for="git_updater_branch">
			...
			<br>
			<span class="description">
				<?php esc_html_e( 'Enter branch name or leave empty for `master`', 'gi-updater' ); ?>
			</span>
		</label>
		<?php
	}
	...
	public function install_theme_complete_actions( $install_actions, $api, $theme_info ) {
		...
		$install_actions['activate'] = '<a href="' . $activate_link . '" class="activatelink"><span aria-hidden="true">' . esc_attr__( 'Activate', 'gi-updater' ) . '</span><span class="screen-reader-text">' . esc_attr__( 'Activate', 'gi-updater' ) . ' &#8220;' . $stylesheet . '&#8221;</span></a>';
		...
		if ( is_network_admin() && current_user_can( 'manage_network_themes' ) ) {
			...
			$install_actions['network_enable'] = '<a href="' . $network_activate_link . '" target="_parent">' . esc_attr_x( 'Network Enable', 'This refers to a network activation in a multisite installation', 'gi-updater' ) . '</a>';
